...
var restjQuery = function( options ) {
	...
	var settings = $.extend({
		...
		namespace: 'sites/', // Default is wp/v2/ for WordPress. For WooCommerce, set it to wc/v1/
		endpoint: '_get_config', // Default: Posts
	...
	console.log('Requested Endpoint: ' + settings.site_url + "/" + settings.namespace + settings.endpoint );
	...
	function restRequest( settings ) {
		...
		if ( auth_headers ) {
			...
			$.ajax({
				async: false,
				url: settings.site_url + "/" + settings.namespace + settings.endpoint,
	...
	function restUploadMedia( settings ) {
		...
		$.ajax({
			async: true,
			url: settings.site_url + "/" + settings.namespace + "media",
			method: 'POST',
			data: settings.postData,
			cache: false,
			contentType: false,
